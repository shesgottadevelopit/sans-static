...
- [ ] (optional) figure out a way to move other files not generated by gulp streams: https://coderwall.com/p/9uzttg/simple-gulp-copy-file-task
...
**Completed:**
- [x] clean up `templates` directory and files

**Notes:**
I was cleaning out my template files and noticed the navigation wasn't rendering. It is because I didn't use `gulp-data` to pass my "master" data to the gulp posts task. All fixed up now.
...
I think I'm going to do some more reading up on `return`ing data in JS. (DONE)
- This was great: https://codeburst.io/javascript-functions-understanding-the-basics-207dbf42ed99
...
**Notes:**
So I figured out how to check this off my list and that can be seen in my latest commits. I'm no longer creating two archive files and I didn't even have to use a gulp plugin for it.

- [x] merge json files into a single file (https://www.npmjs.com/package/gulp-merge-json, https://www.npmjs.com/package/gulp-concat)

I think now that I have most of the functionality ready, I'm going to start cleaning up these starter files. I might also do some housekeeping with my deploy tasks.(DONE)

- [x] copy producction ready files to `dist` and spin up a test server if needed, using those files

I tackled this pretty easily since I had the setup and just needed to clean up my gulpfile, copy some other configs over. I did need a little help trying to exclude my `data` folder which I'm not copying over to production, thanks to [Github](https://github.com/gulpjs/gulp/issues/165) issues.
